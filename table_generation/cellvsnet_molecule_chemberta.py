...
try:
    emb_data = np.load(EMBEDDING_NPZ_FILE, allow_pickle=True)
    emb_ids = emb_data['inst_ids']
    emb_vectors = emb_data['embeddings']
    print(f"Loaded {len(emb_ids)} embeddings.")
except FileNotFoundError:
    print(f"ERROR: Embedding file not found at {EMBEDDING_NPZ_FILE}")
    print("Please run the 'generate_embeddings.py' script first and ensure")
    print("the 'drug_embeddings.npz' file is in the correct location.")
    raise
except Exception as e:
    print(f"Error loading embedding file: {e}")
    raise

embedding_map = dict(zip(emb_ids, emb_vectors))
print("Created embedding map from inst_id to vector.")
...
def process_data_split(df_split, split_name):
    ...
    print(f"Retrieving ChemBERTa embeddings for {split_name} set...")
    chemberta_embeddings = []
    missing_ids = 0
    
    try:
        first_key = next(iter(embedding_map))
        emb_dim = embedding_map[first_key].shape[0]
        zero_vector = np.zeros(emb_dim)
    except StopIteration:
        raise ValueError("Embedding map is empty. Cannot proceed.")

    for inst_id in df_split['inst_id']:
        if inst_id in embedding_map:
            chemberta_embeddings.append(embedding_map[inst_id])
        else:
            chemberta_embeddings.append(zero_vector)
            missing_ids += 1
    
    if missing_ids > 0:
        warnings.warn(f"{missing_ids} inst_ids in {split_name} set were not found in the embedding map. Using zero vectors.")

    chemberta_embeddings = np.array(chemberta_embeddings)
    print(f"Retrieved ChemBERTa embeddings for {split_name}: shape {chemberta_embeddings.shape}")
    ...
    return X_raw, chemberta_embeddings, pert_unit_dummies, pert_time, pert_dose, ignore_time, ignore_dose

X_raw_train, chemberta_embeddings_train, pert_unit_dummies_train, pert_time_train, pert_dose_train, ignore_time_train, ignore_dose_train = process_data_split(df_train, 'train')
X_raw_test, chemberta_embeddings_test, pert_unit_dummies_test, pert_time_test, pert_dose_test, ignore_time_test, ignore_dose_test = process_data_split(df_test, 'test')
...
chemberta_train_scaled = scaler_chemberta.fit_transform(chemberta_embeddings_train.astype(float))
chemberta_test_scaled = scaler_chemberta.transform(chemberta_embeddings_test.astype(float))
print(f"ChemBERTa embeddings scaled: train {chemberta_train_scaled.shape}, test {chemberta_test_scaled.shape}")
...
def build_context_matrix_improved(df_split, chemberta_embeddings_scaled, pert_time, pert_dose, 
    ...
    for i, cell_id in enumerate(valid_cells):
        ...
        C_cell = np.hstack([
            C_continuous_scaled,                         
            chemberta_embeddings_scaled[mask],             
            ignore_time[mask],                            
            ignore_dose[mask],
        ])
...
X_train, C_train, cell_ids_train, scaler_context = build_context_matrix_improved(
    df_train, chemberta_train_scaled, pert_time_train, pert_dose_train,
    ignore_time_train, ignore_dose_train, 'train', is_train=True
)

X_test, C_test, cell_ids_test, _ = build_context_matrix_improved(
    df_test, chemberta_test_scaled, pert_time_test, pert_dose_test,
