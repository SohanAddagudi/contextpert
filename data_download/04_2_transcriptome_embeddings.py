...
# Load gene columns from pre-generated list (created by 04_1_gene_cols.py)
gene_cols_file = os.path.join(DATA_DIR, 'trt_sh_qc_gene_cols.txt')
print(f"  Loading gene columns from: {gene_cols_file}")

with open(gene_cols_file, 'r') as f:
    gene_cols = [line.strip() for line in f if line.strip()]

print(f"  Found {len(gene_cols)} gene expression features (Entrez IDs)")
...
for chunk_df in pd.read_csv(lincs_path, chunksize=args.chunk_size):
    ...
    # Determine metadata columns (all non-gene columns)
    metadata_cols = [col for col in chunk_df.columns if col not in symbol_cols]

    # Extract metadata (preserve all non-gene columns)
    metadata_df = chunk_df[metadata_cols].copy()

    # Extract expression matrix (using gene columns only)
