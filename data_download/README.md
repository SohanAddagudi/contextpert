...
The OpenTargets ETL has been split into per-stage scripts:

```bash
python 02_2_drug_target_pairs.py
python 02_3_disease_target_pairs.py
python 02_4_disease_drug_triples.py
python 02_5_filter_diseases.py
```
...
1. `01_7_brd_to_chembl.py`: Maps BRD compound identifiers to ChEMBL IDs using the ChEMBL web service API.
...
2. `01_8_pert_gene_target.py` / `01_9_trt_sh_targets.py`: Merge ChEMBL-mapped LINCS perturbations with their gene targets via the OpenTargets drug→target table.

**Input Files:**
- Cancer target drug data (following the instructions of the OpenTargets dataset processing pipeline)
- LINCS data with ChEMBL mappings (generated by `01_7_brd_to_chembl.py`)
